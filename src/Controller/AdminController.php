...
class AdminController extends AbstractController
{
    ...
    public function dashboard(
        ...
        $sponsorRepo = $entityManager->getRepository(Sponsor::class);
        $participationRepo = $entityManager->getRepository(Participation::class);

        $totalFeedbacksQb = $participationRepo->createQueryBuilder('p')
            ->select('COUNT(p)')
            ->where("p.note_satisfaction IS NOT NULL OR (p.feedback_commentaire IS NOT NULL AND p.feedback_commentaire <> '')");

        $moyenneSatisfaction = $participationRepo->createQueryBuilder('p')
            ->select('AVG(p.note_satisfaction)')
            ->where('p.note_satisfaction IS NOT NULL')
            ->getQuery()
            ->getSingleScalarResult();

        $statsEvenements = [
            'total_evenements' => $evenementRepo->count([]),
            'evenements_a_venir' => $evenementRepo->count(['statut' => 'a_venir']),
            'evenements_en_cours' => $evenementRepo->count(['statut' => 'en_cours']),
            'evenements_termines' => $evenementRepo->count(['statut' => 'termine']),
            'evenements_annules' => $evenementRepo->count(['statut' => 'annule']),
            'total_sponsors' => $sponsorRepo->count([]),
            'total_participations' => $participationRepo->count([]),
            'participations_en_attente' => $participationRepo->count(['statut' => 'attente']),
            'participations_confirmees' => $participationRepo->count(['statut' => 'confirme']),
            'participations_annulees' => $participationRepo->count(['statut' => 'annule']),
            'total_feedbacks' => $totalFeedbacksQb->getQuery()->getSingleScalarResult(),
            'moyenne_satisfaction' => $moyenneSatisfaction !== null ? round((float) $moyenneSatisfaction, 2) : null,
            'dernieres_participations' => $participationRepo->createQueryBuilder('p')
                ...
                ->getQuery()
                ->getResult(),
            'derniers_feedbacks' => $participationRepo->createQueryBuilder('p')
                ->leftJoin('p.evenement', 'e')
                ->leftJoin('p.etudiant', 'u')
                ->addSelect('e', 'u')
