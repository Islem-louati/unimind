...
class SuiviTraitementController extends AbstractController
{
    ...
    #[Route('/selectionner-traitement', name:
'app_suivi_traitement_select_traitement', methods: ['GET'])]
    public function selectTraitement(EntityManagerInterface
$entityManager): Response
    {
        ...
        if ($this->isGranted('ROLE_PSYCHOLOGUE')) {
            $traitements =
$entityManager->getRepository(Traitement::class)->createQueryBuilder('t')
...
$entityManager->getRepository(Traitement::class)->createQueryBuilder('t')
    ...
    public function index(SuiviTraitementRepository
$suiviTraitementRepository): Response
    {
        $user = $this->security->getUser();
...
$this->isGranted('ROLE_RESPONSABLE_ETUDIANT')) {
    ...
    #[Route('/a-valider', name: 'app_suivi_traitement_a_valider',
methods: ['GET'])]
    #[IsGranted('ROLE_PSYCHOLOGUE')]
    public function aValider(SuiviTraitementRepository
$suiviTraitementRepository): Response
    {
        $user = $this->security->getUser();
        $suivis =
$suiviTraitementRepository->findNonValidesByPsychologue($user);
    ...
    #[Route('/nouveau/{traitement_id}', name:
'app_suivi_traitement_new', methods: ['GET', 'POST'])]
    public function new(Request $request, EntityManagerInterface
$entityManager, int $traitement_id): Response
    {
        $user = $this->security->getUser();
        ...
        $traitement =
$entityManager->getRepository(Traitement::class)->find($traitement_id);
...
erreurs. Veuillez corriger les champs obligatoires.');
    ...
    #[Route('/{id}/modifier', name: 'app_suivi_traitement_edit',
methods: ['GET', 'POST'])]
    public function edit(Request $request, SuiviTraitement $suivi,
...
pas modifier ce suivi');
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('app_suivi_traitement_show',
['id' => $suivi->getId()]);
        }
...
erreurs. Veuillez corriger les champs obligatoires.');
    ...
    #[Route('/{id}/effectuer', name: 'app_suivi_traitement_effectuer',
methods: ['POST'])]
    public function effectuer(Request $request, SuiviTraitement
$suivi, EntityManagerInterface $entityManager): Response
    {
        $user = $this->security->getUser();
...
pas modifier ce suivi');
        ...
        if ($this->isCsrfTokenValid('effectuer'.$suivi->getId(),
$request->request->get('_token'))) {
        ...
        return $this->redirectToRoute('app_traitement_show', ['id' =>
$suivi->getTraitement()->getId()]);
    }

    #[Route('/{id}/valider', name: 'app_suivi_traitement_valider',
methods: ['POST'])]
    #[IsGranted('ROLE_PSYCHOLOGUE')]
    public function valider(Request $request, SuiviTraitement $suivi,
EntityManagerInterface $entityManager): Response
    {
        $user = $this->security->getUser();
        ...
        if (!$this->canValidateSuivi($suivi, $user)) {
            throw $this->createAccessDeniedException('Vous ne pouvez
pas valider ce suivi');
        }

        if ($this->isCsrfTokenValid('valider'.$suivi->getId(),
$request->request->get('_token'))) {
            if (!$suivi->isEffectue()) {
                $this->addFlash('error', 'Un suivi doit être effectué
avant d\'être validé.');
            } else {
                $suivi->setValide(true);
                $entityManager->flush();

                $this->addFlash('success', 'Le suivi a été validé avec
succès.');
            }
        }
...
$suivi->getTraitement()->getId()]);
    ...
    #[Route('/{id}/supprimer', name: 'app_suivi_traitement_delete',
methods: ['POST'])]
    public function delete(Request $request, SuiviTraitement $suivi,
EntityManagerInterface $entityManager): Response
    {
        $user = $this->security->getUser();
        ...
        if (!$this->canEditSuivi($suivi, $user)) {
            throw $this->createAccessDeniedException('Vous ne pouvez
pas supprimer ce suivi');
        }

        if ($this->isCsrfTokenValid('delete'.$suivi->getId(),
$request->request->get('_token'))) {
...
$suivi->getTraitement()->getId()]);
    ...
    private function canEditSuivi(SuiviTraitement $suivi, User $user): bool
    {
        ...
        if ($this->isGranted('ROLE_PSYCHOLOGUE') &&
$suivi->getTraitement()->getPsychologue() === $user) {
            return true;
        }

        if ($this->isGranted('ROLE_ETUDIANT') &&
$suivi->getTraitement()->getEtudiant() === $user) {
            return true;
        }
...
$traitement->getEtudiant() === $user) {
    ...
    #[Route('/{id}', name: 'app_suivi_traitement_show', methods:
['GET'], requirements: ['id' => '\d+'])]
    public function show(int $id, SuiviTraitementRepository
$suiviTraitementRepository): Response
    {
