...
class User implements UserInterface, PasswordAuthenticatedUserInterface
{
    ...
    {
        // Vérifier si le rôle est un rôle Symfony (avec préfixe ROLE_)
        if (str_starts_with($role, 'ROLE_')) {
            return in_array($role, $this->getRoles(), true);
        }
        
        // Sinon, vérifier avec le préfixe ROLE_ ajouté
        return in_array('ROLE_' . strtoupper(str_replace(' ', '_', $role)), $this->getRoles(), true);
    }
    ...
    {
        // Not needed when using modern password hashers (bcrypt, argon2)
        return null;
    }
